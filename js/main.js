...
function sortiraj() {
    let val= this.value;
           ...
           if(localStorage.getItem("filtriraniProizvodi") && (nizMarki.length!=0 || nizRamMem.length!=0 || nizIntMem.length!=0 || nizRezolucija.length!=0)){
...
var nizRezolucija=[];
var nizIntMem=[];
function svaFiltriranja(){
function filtriranjePoMarci(){
    
    let val=this.value;
    document.getElementById("search").value="";
    document.getElementById("sortiraj").value=0;
...
function filtriranjeSearch(){
    ...
    return {
        ...
        filtriranjePoIntMem,
        filtriranjePoRamMem,
        filtriranjePoRezoluciji,
        filtriranjeSearch
    }
}
function tip(tip){
    var filtriranje=svaFiltriranja();
    localStorage.removeItem("filtriraniProizvodiPoRamMem");
    localStorage.removeItem("filtriraniProizvodiPoIntMem");
    localStorage.removeItem("filtriraniProizvodiPoRez");
    ...
    window.onload=function(){
        $.ajax({
            ...
            success:function(data){
                ...
                $("input[name='robnaMarka']").click(filtriranje.filtriranjePoMarci);
                $("input[name='ramMemorija']").click(filtriranje.filtriranjePoRamMem);
                $("input[name='internaMemorija']").click(filtriranje.filtriranjePoIntMem);
                $("input[name='rezolucija']").click(filtriranje.filtriranjePoRezoluciji);
            },
            error:function(err){
                console.error(err);
            }
        ...
        document.getElementById("search").addEventListener("keyup", filtriranje.filtriranjeSearch);
    }
}
